...
function CarouselCard({blog}) {
  ...
  return (
        <div className='flex flex-col w-full flex-wrap gap-2 items-start justify-between font-manrope absolute h-full p-6 text-white' key={blog?._id} style={{textOverflow:'ellipsis'}}>
            <img draggable={false} src={blog?.image} className='w-full h-full bg-gradient-to-b from-black/75 to-black/85 object-cover object-center absolute left-0 top-0'/>
            <div className='absolute w-full top-0 left-0 bg-gradient-to-b from-black/60 to-black/70 mix-blend-multiply h-full'></div>
            <h3 className='uppercase tracking-wider text-sm text-white border-b-2 border-green-600' style={{zIndex:1}}>Latest Blogs</h3>
            <h2 className='font-manrope font-extrabold md:p-4 md:max-w-5xl leading-snug text-3xl md:text-6xl animate-form' style={{color:'white'}}>{blog?.title}</h2>
            <p className='animate-form text-sm md:text-lg md:p-4 md:max-w-6xl'>{blog?.subtitle}</p>
            <div className='w-full flex items-center justify-between'>
            <p className='text-xs text-white capitalize text-center p-2 rounded-full border border-green-600 animate-form'>{blog?.genre}</p>
