...
function Search() {
  ...
  return (
    <div className={`container w-full pt-4 px-4 md:px-0 font-manrope transition-all flex flex-col ${!searchOpen && 'h-0 pt-0 p-0 opacity-0'}`}>
        ...
        <input type={'text'} value={query} onChange={handleChange} placeholder={`Search ${pathname.includes('blog') ? 'Blogs' : 'Doctors'}`} className='p-2 w-full border-b-2 focus:outline-none focus:border-black transition-colors' required ></input>
        ...
        {result.length>0 && <ul className='list-group list-group-flush bg-transparent' style={{display: showSearchResults ? 'block' : 'none'}}>
