...
function Footer() {
  ...
  return (
    <div className='flex flex-col gap-4 md:gap-8 text-gray-300 font-manrope container mix-blend-difference'>
    <div className='w-full flex flex-col md:flex-row items-start uppercase justify-between '>
        <div className='flex flex-col items-start gap-2'>
            <Brand size={3} color={'white'}/>
            <p className='font-medium capitalize text-md '>Healthcare Made Easy!</p>
        </div>
        ...
        <ul className='flex flex-col items-start gap-1'> 
            <NavLink  className='font-light text-sm text-white/85' to={'/blog'}>Healthcare Blogs</NavLink>
            <NavLink  className='font-light text-sm text-white/85' to={`/profile/${userInfo?._id}`}>Profile</NavLink>
        </ul>
        {userInfo && <ul className='flex flex-col items-start gap-1'> 
            {userInfo?.role ==='Doctor' && <NavLink  className='font-light text-sm text-white/85' to={'/add'}>Add New Blog</NavLink>}
            {!userInfo && <NavLink  className='font-light text-sm text-white/85' to={`/user-authentication/login`}>Login</NavLink>}
        </ul>}
        </div>
        <div className='flex flex-col md:flex-row gap-2 w-full items-center font-manrope'>
            ...
            <form className='w-full flex-1 flex rounded-full overflow-hidden border'>
                ...
                <button type='submit' className='bg-pink-500 text-b px-4 text-sm rounded-full'>Submit</button>
            </form>
        </div>
        </div>
    </div>
    ...
    <div>
        <p className='text-gray-500 text-sm text-center'>&copy; {new Date().getFullYear()} MedEasy. All rights reserved.</p>
    </div>
    </div>
  )
}
