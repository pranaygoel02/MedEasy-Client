  ...
  return (
    ...
    <div className='flex flex-col w-full flex-wrap font-manrope py-8'>
        <div id='profileHead' ref={profileRef} className='w-full fixed gap-4 left-0 px-2 md:px-16 py-2 -top-16 md:pt-4 h-16 bg-white border-b-2 border-gray-100 items-center md:items-center overflow-hidden object-center flex flex-row transition-all'>
          <img draggable={false} src={id === userInfo?._id ? (userInfo?.img || Avatar) : (profileBlogList && profileBlogList[0]?.author?.img || Avatar)} className='h-12 md:h-full object-cover rounded-full p-0' style={{aspectRatio:1/1}}/>
        ...
        <div className='w-full h-24 md:h-40 items-center md:items-start overflow-hidden object-center flex flex-row'>
          <img draggable={false} src={id === userInfo?._id ? (userInfo?.img || Avatar) : (profileBlogList && profileBlogList[0]?.author?.img || Avatar)} className='h-24 md:h-full object-cover rounded-full p-0' style={{aspectRatio:1/1}}/>
